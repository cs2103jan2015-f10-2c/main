...
namespace iLogicTest
{
	...
	{
	public:
		TEST_METHOD(AssignPriorityTest1)
		{
			Logic testLogic;
			Schedule testSchedule;
			Item testItem;
			testItem.setItemID(13);
			unsigned int addedItemId = testLogic.addTask(testItem);
			testLogic.assignPriorityToNewTask('h');
			char assignedPriority = testLogic.getSchedule()[0].getPriority();
			Assert::AreEqual('h', assignedPriority);
		}
	};*/
}